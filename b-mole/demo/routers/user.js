...
userRouter.get('/updateCart',(req,res)=>{
	var obj=req.query;
	var sql='insert into cart values (null,"../../img/details.png",?,?,?,?,?)'
	pool.query(sql,[obj.cartPImg,obj.cartTitle,obj.cartPrice,obj.cartNumber,obj.uid],(err,result)=>{
		if(err) throw err;
		if(result.affectedRows>0){
			console.log(result)
			res.send({code:'200',msg:'success'});
